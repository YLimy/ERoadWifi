			...
			try {
				if (NetUtil.hasNetwork(context)) {
					...
					if (obj instanceof NetStatus) {
						Map<String, String> map_sp_clean = new HashMap<String, String>();
						map_sp_clean.put(IConstant.LOGIN_USER_NAME, "");
						map_sp_clean.put(IConstant.NICKNAME, "");
						CommonUtil.saveSharedPreferences(context, "sp", map_sp_clean);
						//跳转到登录界面
						Intent intent = new Intent(context,
								LoginFragmentActivity.class);
						Bundle bundle = new Bundle();
						bundle.putBoolean(IConstant.IK_BASE_LOGIN_NOT, true);
