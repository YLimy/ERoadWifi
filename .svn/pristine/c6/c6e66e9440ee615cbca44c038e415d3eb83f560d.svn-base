...
public class DownLoadTask implements Runnable {
		...
		try {
			...
			while (threadCount != countSucess && flag && threadCount != stop) {
				...
				case DownMessage.MSG_SUCCESS:
					countSucess++;
					LoggerUtil.d(TAG, "线程Id=" + msg.threadid + "已完成下载");
					break;
				case DownMessage.MSG_UPDATE:
					if (listener != null) {
						listener.update(length, msg.data, msg.threadid, url);
					}
					break;
			...
			LoggerUtil.d(TAG, "耗时" + (System.currentTimeMillis() - starttime));

		} catch (Throwable e) {
			if (e instanceof ConnectException) {
				notifyListenerError(ERROR_CONNECTEXCEPTION);
				LoggerUtil.e(TAG, "连接异常", e);
			} else if (e instanceof FileNotFoundException) {
				notifyListenerError(ERROR_FILENOTFOUND);
				LoggerUtil.e(TAG, "文件路径错误", e);
			} else if (e instanceof SocketTimeoutException) {
				notifyListenerError(ERROR_FILENOTFOUND);
				LoggerUtil.e(TAG, "连接超时", e);
			} else {
				LoggerUtil.e(TAG, e);
			}
			...
			conn = null;
			if (listener != null)
				listener.downLoadFinish(countSucess,url);
			Release();
		}
	}
	...
	private void notifyListenerError(int type) {
		if (listener != null)
			listener.downLoadError(type,url);
	}
...
//				conn.setRequestMethod("POST");
				...
				if (responseCode == HttpURLConnection.HTTP_PARTIAL || responseCode == HttpURLConnection.HTTP_OK) {
					...
					LoggerUtil.d(TAG, "error==>http");
					sendMessage(DownMessage.MSG_ERROR, 10);
				}
			} catch (Exception e) {
				e.printStackTrace();
				try {
					LoggerUtil.d(TAG, "error==>exception");
		...
		private void sendMessage(int type, int data) throws InterruptedException {
			msg = bufferMsg.poll();
			if (msg == null)
				msg = new DownMessage(type, data, threadid, url);
			else {
				msg.type = type;
				msg.data = data;
	...
	public static class DownMessage {
		...
		public DownMessage(int type, int data, int threadid, String url) {
			this.type = type;
			this.data = data;
			this.threadid = threadid;
			this.url = url;
	...
	public static interface DownlaodListener {
		void update(int total, int len, int threadid,String url);

		void downLoadFinish(int totalSucess,String url);

		void downLoadError(int type,String url);
	}
}
